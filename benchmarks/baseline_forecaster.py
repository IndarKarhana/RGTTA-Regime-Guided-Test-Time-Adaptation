...
class BaselineForecaster:
    """
    Baseline forecaster that ALWAYS trains from scratch.
    No checkpointing, no regime detection, no weight reuse.
    """
    ...
    def __init__(
        ...
        dropout: float = 0.1,
        device: str = "cpu",
        # Model selection (same interface as TTA policies)
    ...
    def fit(
        ...
        """
        Train model FROM SCRATCH on the provided data.
        Always creates a new model - no weight reuse.
        """
        ...
        # ALWAYS create NEW model (baseline behavior)
        ...
        # Clamp inputs for numerical stability (data scaled to [-1, 1])
        ...
        if best_model_state is not None:
            ...
            if not has_nan_in_state:
                self.model.load_state_dict(best_model_state)
            else:
                logging.warning("⚠️ Best model state contains NaN, using final weights")
