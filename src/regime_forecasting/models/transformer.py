...
class TimeSeriesTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.output_projection = nn.Sequential(
            nn.Linear(hidden_dim, hidden_dim), nn.GELU(), nn.Dropout(dropout), nn.Linear(hidden_dim, forecast_horizon)
        )

        # Component heads (for compatibility)
        self.trend_head = nn.Sequential(
            nn.Linear(hidden_dim, hidden_dim // 2), nn.GELU(), nn.Linear(hidden_dim // 2, forecast_horizon)
        )
    ...
    def forward(
        self, target_seq: torch.Tensor, exog_seq: torch.Tensor = None, return_components: bool = False
    ) -> torch.Tensor:
        """
        ...
        if return_components:
            ...
            return {"forecast": forecast, "trend": trend, "seasonal": seasonal, "residual": forecast - trend - seasonal}
...
def regime_aware_loss(
    y_true: torch.Tensor, y_pred: torch.Tensor, components: dict = None, alpha: float = 0.3
) -> torch.Tensor:
    """
    ...
    if components is not None:
        ...
        seasonal_loss = F.smooth_l1_loss(components["seasonal"], seasonal_target, reduction="mean", beta=1.0)
