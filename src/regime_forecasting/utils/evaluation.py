...
class EvaluationTracker:
    ...
    def add_regime_evaluation(
        self, regime_id: str, y_true: np.ndarray, y_pred: np.ndarray, timestamp: Optional[str] = None
    ):
        ...
        evaluation_record = {"timestamp": timestamp, "metrics": metrics, "n_samples": len(y_true)}

        self.regime_metrics[regime_id].append(evaluation_record)
        self.overall_metrics.append({"regime_id": regime_id, **evaluation_record})

        logger.info(
            f"📊 Evaluation - Regime {regime_id}: MAPE={metrics.get('weighted_mape', 0):.2f}%, RMSE={metrics.get('rmse', 0):.4f}"
        )
