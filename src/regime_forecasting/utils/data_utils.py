...
class DataPreprocessor:
    ...
    def transform(self, df: pd.DataFrame, target_col: str = "y", exog_cols: Optional[list] = None) -> pd.DataFrame:
        ...
        if len(new_vals) > 0:
            ...
            if new_min < old_min or new_max > old_max:
                ...
                if rng > 1e-8:
                    ...
                    updated = True
                    logger.info(
                        f"📏 Scaler updated: y range [{old_min:.2f}, {old_max:.2f}] → [{exp_min:.2f}, {exp_max:.2f}]"
                    )
    ...
    def decompose_series(self, y: np.ndarray, model: str = "additive") -> Dict[str, np.ndarray]:
        """
        ...
        if len(y) < 2 * self.season_length:
            logger.warning(
                f"⚠️ Series too short for decomposition. Need at least {2 * self.season_length} points, got {len(y)}"
            )
            return {"trend": np.full_like(y, np.mean(y)), "seasonal": np.zeros_like(y), "residual": y - np.mean(y)}
        ...
        try:
            ...
            decomposition = seasonal_decompose(ts, model=model, period=self.season_length, extrapolate_trend="freq")
            ...
            return {"trend": trend, "seasonal": np.zeros_like(y), "residual": residual}


def create_lagged_features(
    df: pd.DataFrame, target_col: str = "y", lags: Optional[list] = None, season_length: int = 12
) -> pd.DataFrame:
    """
...
def prepare_sequences(
    ...
    logger.info(
        f"📝 Created {len(X_target)} sequences (seq_len: {sequence_length}, "
