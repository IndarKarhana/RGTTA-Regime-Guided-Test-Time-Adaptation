...
class CorrectedRegimeForecaster:
    ...
    def __init__(
        ...
        self.memory_module = MemoryModule(storage_path=storage_path, similarity_threshold=similarity_threshold)
        self.exog_cols = []
    ...
    def compute_dynamic_threshold(self, y: np.ndarray) -> float:
        ...
        self.threshold_history.append(
            {"threshold": threshold, "cv": cv, "trend_strength": trend_strength, "acf1": acf1}
        )
    ...
    def update_with_new_data(
        ...
        feature_window = min(len(new_df), self.season_length * 3)
        new_distribution = self.extract_distribution_features(new_df.tail(feature_window)["y"].values)
        ...
        if match_result is not None:
            ...
            if self.model is not None:
                previous_full_state = {k: v.cpu().clone() for k, v in self.model.state_dict().items()}
            ...
            new_full_state = {k: v.cpu().clone() for k, v in self.model.state_dict().items()}
            ...
            if previous_full_state is not None:
                ...
                if len(partial_data) >= self.sequence_length + self.forecast_horizon:
                    ...
                    if partial_train_result["status"] == "completed":
                        ...
                        partial_distribution = self.extract_distribution_features(partial_data["y"].values)
                        ...
                        partial_state = {k: v.cpu().clone() for k, v in self.model.state_dict().items()}
                else:
    ...
    def _train_model_from_scratch(
        ...
        return self._run_training_loop(X_target, X_exog, y, epochs, batch_size, learning_rate, validation_split)
        ...
        for epoch in range(epochs):
            ...
            if val_idx:
                ...
                if (epoch + 1) % 10 == 0:
                    ...
                    logger.info(
                        f"Epoch {epoch + 1}/{epochs} - Train: {avg_train_loss:.4f}, Val: {val_loss:.4f}, LR: {current_lr:.6f}"
                    )
                self.model.train()
            else:
    ...
    def _save_to_memory(
        ...
        weights = {k: v.cpu().clone() for k, v in self.model.state_dict().items()} if self.model else None
