...
class Checkpoint:
    ...
    def __init__(self, regime_id: str, weights: Dict[str, Any], regime_features: np.ndarray, metadata: Dict[str, Any]):
...
class MemoryModule:
    ...
    def save_checkpoint(
        ...
        if not self._validate_weights(weights):
            logger.warning("⚠️ Refusing to save checkpoint with NaN/Inf weights")
            return None
    ...
    def find_similar_regime(self, query_features: np.ndarray) -> Optional[Dict[str, Any]]:
        ...
        if best_similarity >= self.similarity_threshold:
            ...
            logger.info(
                f"🆕 No similar regime found (best similarity: {best_similarity:.3f} < {self.similarity_threshold})"
            )
            return None
