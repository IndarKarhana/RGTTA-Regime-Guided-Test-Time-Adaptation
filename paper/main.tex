...
Regime-guided policies achieve the lowest MSE in \textbf{156 of 224} seed-averaged experiments (\textbf{69.6\%}), with RG-EWC winning 30.4\% and RG-TTA winning 29.0\%. On real-world benchmarks, RG-TTA reduces MSE by 4.4\% vs TTA while running 5.5\% faster; RG-EWC reduces MSE by 14.1\% vs standalone EWC. The regime-guidance layer composes naturally with existing TTA methods, improving accuracy without modifying the underlying forecaster.\footnote{Code and data: \repourl}
...
Time series forecasting in production is inherently incremental: data arrives in batches, and the system must decide how to update its model~\citep{lim2021timeseries,benidis2023deep}. Test-time adaptation (TTA)~\citep{liang2024tta} has emerged as a practical approach, applying a fixed number of gradient steps to each incoming batch. Recent work has improved upon fixed TTA: TAFAS~\citep{kim2025tafas} freezes the source model and learns calibration modules (GCMs) that adapt predictions, while DynaTTA~\citep{grover2025dynatta} dynamically adjusts the learning rate based on prediction-error z-scores and embedding drift.
...
\begin{enumerate}[nosep]
  ...
  \item A \textbf{comprehensive benchmark} spanning 6 update policies, 4 compact model architectures, 14 datasets, 4 forecast horizons, and 3 random seeds (672 experiments)---the most thorough controlled comparison of TTA strategies for streaming time series forecasting to date.
\end{enumerate}
...
TAFAS~\citep{kim2025tafas} is a strong baseline at short horizons (H$\leq$192) where its frozen-source GCM calibration excels, but its performance collapses at longer horizons (H=336/720, $+$100--400\% vs TTA in our experiments)---a consequence of its design for sliding-window evaluation. Because our study spans H$\in$\{96,192,336,720\}, we include TAFAS as a reference policy but exclude it from the primary 6-policy comparison.

\paragraph{Continual learning and catastrophic forgetting.}
Continual learning~\citep{parisi2019continual,mccloskey1989catastrophic} addresses the problem of updating a model on sequential tasks without forgetting old knowledge. EWC~\citep{kirkpatrick2017ewc} penalises changes to parameters important for previous tasks via the diagonal Fisher Information Matrix. Synaptic Intelligence~\citep{zenke2017continual} tracks parameter importance online. Experience replay~\citep{rolnick2019experience} stores and re-uses past data samples. Progressive networks~\citep{rusu2016progressive} add capacity for new tasks. Our approach is complementary: rather than regularising a single model, RG-TTA maintains a \emph{library of checkpoints} indexed by distributional features and selects the appropriate one. When combined with EWC (RG-EWC), the regime-guided adaptation operates under EWC regularisation ($\lambda = 400$), preventing catastrophic forgetting while benefiting from checkpoint reuse and similarity-modulated learning rates.

\paragraph{Concept drift and regime switching.}
Concept drift detection~\citep{gama2014concept,lu2018concept,aminikhanghahi2017survey} identifies \emph{that} a distribution change occurred. Our approach extends this by detecting not just \emph{change} but \emph{recurrence}---whether the new distribution matches a previously-seen regime. Hamilton's Markov-switching model~\citep{hamilton1989} uses latent states for regime transitions but assumes a specific generative process. RG-TTA makes no parametric assumption: ``regime'' is defined operationally by a 5-dimensional distributional feature vector, and matching uses non-parametric statistical tests.

\paragraph{Time series forecasting architectures.}
Recent neural forecasters---Informer~\citep{zhou2021informer}, Autoformer~\citep{wu2021autoformer}, iTransformer~\citep{liu2024itransformer}, PatchTST~\citep{nie2023patchtst}, DLinear~\citep{zeng2023dlinear}---advance accuracy but do not address the \emph{when-to-adapt} question. We evaluate RG-TTA across four compact model architectures spanning three model families---recurrent (GRU~\citep{cho2014gru}), attention-based (iTransformer, PatchTST), and linear (DLinear)---to validate model-agnosticism. Notably, iTransformer and PatchTST are the same modern Transformer architectures targeted by DynaTTA~\citep{grover2025dynatta}.
...
\paragraph{Kolmogorov--Smirnov similarity.} Based on the two-sample KS statistic on the raw values:
\begin{equation}
  \text{sim}_{\text{KS}} = 1 - D_n, \quad D_n = \sup_x |F_{\mathbf{q}}(x) - F_{\mathbf{s}}(x)|.
\end{equation}

\paragraph{Wasserstein-1 similarity.} Normalised earth-mover's distance:
\begin{equation}
...
\begin{equation}
  ...
  \label{eq:ensemble}
\end{equation}
The statistical tests (KS, Wasserstein) use the full empirical distribution and receive higher weight; the feature vector provides complementary higher-order moment information; the variance ratio captures volatility regime shifts.
...
\IF{$\text{sim}^* \geq 0.75$} \hfill \COMMENT{Checkpoint candidate}
  \STATE $\ell_{\text{ckpt}} \leftarrow \mathcal{L}(f_{\theta^*_{\text{ckpt}}}(X_t), y_t)$
  \IF{$\ell_{\text{ckpt}} < g \cdot \ell_{\text{curr}}$} \hfill \COMMENT{Loss gate: $g = 0.70$ ($\geq$30\% improvement)}
...
\FOR{$k = 1$ to $K_{\max}$}
  ...
  \STATE $\mathcal{L} = \text{SmoothL1}(\hat{y}, y_t)$~\citep{huber1964robust} \hfill \COMMENT{+ optional EWC/DynaTTA terms}
...
RG-EWC adds EWC regularisation~\citep{kirkpatrick2017ewc} to the RG-TTA framework. The total loss becomes:
\begin{equation}
...
RG-DynaTTA combines \emph{proactive} regime detection (RG-TTA) with \emph{reactive} shift-magnitude sensing (DynaTTA~\citep{grover2025dynatta}). Instead of the similarity-based smooth LR (Eq.~\ref{eq:lr}), DynaTTA's sigmoid formula computes the actual LR based on three shift metrics (prediction-error z-score, RTAB/RDB embedding distances). This creates a two-level controller: RG-TTA provides checkpoint reuse and loss-driven early stopping; DynaTTA provides the continuous LR within $[\alpha_{\min}, \alpha_{\max}]$ based on current error signals.
...
We provide formal analysis of RG-TTA's three design pillars: frozen-backbone adaptation (\S\ref{sec:theory_gen}), similarity-guided initialisation (\S\ref{sec:theory_adapt}), and the ensemble similarity metric (\S\ref{sec:theory_metric}). Together, these results establish that RG-TTA's mechanisms are principled---not merely heuristic---and yield quantitative conditions under which regime-guidance provably improves upon fixed-strategy TTA.
...
\begin{proof}
Under $\mu$-strong convexity and $L$-smoothness, gradient descent with $\alpha \leq 2/(\mu + L)$ satisfies the standard contraction~\citep{nesterov2004introductory}:
...
\begin{proof}
The proof follows the standard Rademacher complexity argument~\citep{bartlett2002rademacher}. Since $g_{\psi^*}$ is fixed, the hypothesis class $\mathcal{F}_{\text{frozen}}$ is a linear class in the representation space $g_{\psi^*}(\mathcal{X})$. By Talagrand's contraction lemma and the Rademacher complexity of linear classes with bounded Frobenius norm~\citep{bartlett2002rademacher}, we obtain Eq.~\ref{eq:rademacher}. The generalisation bound Eq.~\ref{eq:gen_bound} follows from McDiarmid's inequality.
\end{proof}
...
\paragraph{Real-world benchmarks (6).} ETTh1, ETTh2 (hourly, 17,420 rows), ETTm1, ETTm2 (15-minute, 69,680 rows)~\citep{zhou2021informer}, Weather (52,696 rows, 21 features), and Exchange (7,588 rows, 8 currencies). These are standard benchmarks used by DynaTTA~\citep{grover2025dynatta} and TAFAS~\citep{kim2025tafas}; we overlap on 6 of their 7 datasets (missing only Illness).
...
\texttt{itransformer} & iTransformer~\citep{liu2024itransformer} & ${\sim}123$K & Attention \\
\texttt{patchtst} & PatchTST~\citep{nie2023patchtst} & ${\sim}192$K & Attention \\
\texttt{dlinear} & DLinear~\citep{zeng2023dlinear} & ${\sim}37$K--$1.2$M & Linear \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{table}[t]
\centering
\caption{Head-to-head: each baseline vs its regime-guided variant. $\Delta$MSE is average relative change (negative = RG-variant is better). Wins = experiments where the RG-variant achieves lower MSE. $p$-values from one-sided Wilcoxon signed-rank tests~\citep{wilcoxon1945} with Bonferroni correction ($\alpha = 0.05/3$).}
\label{tab:pairwise}
\small
...
\begin{figure}[t]
\centering
\includegraphics[width=0.85\textwidth]{figures/pairwise_improvement.pdf}
...
\paragraph{Real-world vs synthetic win rates.} A potential concern is that the overall 69.6\% win rate is inflated by synthetic datasets designed to favour regime-detection. Separating the two categories shows this is not the case: regime-guided policies win \textbf{67/96 (69.8\%)} on real-world data and \textbf{89/128 (69.5\%)} on synthetic data---nearly identical rates. The regime-guidance benefit is not an artefact of synthetic design; it generalises to standard real-world benchmarks.

Figure~\ref{fig:heatmap} provides a per-dataset heatmap of win rates.

\begin{figure}[t]
\centering
\includegraphics[width=0.85\textwidth]{figures/dataset_heatmap.pdf}
...
\begin{figure}[t]
\centering
\includegraphics[width=0.95\textwidth]{figures/horizon_analysis.pdf}
...
\subsection{Similarity Threshold Sensitivity}

We sweep the checkpoint-loading similarity threshold across $\{0.65, 0.70, 0.75, 0.80, 0.85, 0.90\}$ on 4 representative datasets (ETTh1, ETTm1, synth\_recurring, synth\_shock\_recovery). The current value of 0.75 is deliberately conservative: combined with the loss gate ($g = 0.70$), checkpoint loading activates on only $\sim$2.4\% of batches across the full benchmark (159 of 6{,}672 batches for RG-TTA). At 0.90, checkpoint reuse essentially never fires (max observed similarity $\approx 0.86$); at 0.65, too many low-quality checkpoints pass, increasing MSE by 3--5\%. Importantly, the low loading rate does not diminish RG-TTA's gains---the primary improvement comes from the similarity-modulated learning rate and loss-driven early stopping, not from checkpoint reuse (see \S\ref{sec:component_contribution}).
...
\begin{enumerate}[nosep]
  ...
  \item \textbf{Threshold sensitivity.} The checkpoint-loading threshold (0.75) and loss gate (0.70) are tuned via grid search. Learned thresholds could improve robustness.
  ...
  \item \textbf{Architecture-dependent frozen backbone.} Frozen-backbone TTA benefits recurrent and linear models but is less effective for attention-based architectures. Exploring adapter modules~\citep{houlsby2019adapters} is a promising direction.
  \item \textbf{No retrain baseline.} Full retraining is prohibitively slow ($\sim$40$\times$ TTA) and was excluded from the primary comparison. Prior experiments show retrain accuracy is comparable to or worse than TTA on these datasets.
\end{enumerate}
...
The practical recommendation is conditional: RG-TTA excels when data streams exhibit recurring distributional regimes (ETT electricity data, periodic processes); for continuously drifting data without recurrence (Weather, financial random walks), reactive methods suffice. The regime-guidance layer is model-agnostic in interface requirements, though the frozen-backbone adaptation paradigm is most effective on recurrent and linear architectures.
...
\midrule
ETTh1, ETTh2 & \citet{zhou2021informer} & 17,420 & Hourly & 24 \\
ETTm1, ETTm2 & \citet{zhou2021informer} & 69,680 & 15-min & 96 \\
Weather & Wetterstation & 52,696 & 10-min & 144 \\
Exchange & \citet{lai2018modeling} & 7,588 & Daily & 5 \\
\midrule
...
\begin{verbatim}
PYTHONPATH=.:src:benchmarks python benchmarks/run_unified_benchmark.py \
...
Dependencies: Python $\geq$ 3.9, PyTorch $\geq$ 2.0, scipy, statsmodels, pandas, scikit-learn. Full list in \texttt{pyproject.toml}.
